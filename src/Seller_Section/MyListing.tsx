...
const MyListing: React.FC<MyListingsModalProps> = ({ onClose }) => {
  ...
  const filteredListings = listings.filter((listing) => {
    ...
    const matchesStatus =
      ...
      (listing.status && listing.status.toUpperCase() === statusFilter);
    const listingCategory = getCategoryLabel(listing);
    const matchesCategory =
      categoryFilter === "All Categories" ||
      listingCategory.toLowerCase() === categoryFilter.toLowerCase();
          ...
          <div className="px-8 py-6 border-b border-gray-100 flex items-center justify-between shrink-0">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                Manage your active and past recyclable material listings.
              </p>
            </div>
            <button
              onClick={onClose}
            ...
            <div className="space-y-4">
              {isLoading ? (
                ...
                filteredListings.map((listing, index) => {
                  ...
                  const displayStatus = listing.status
                  ...
                  return (
                    <div
                      ...
                      className="flex flex-col md:flex-row md:items-center justify-between p-5 rounded-2xl border border-gray-100 hover:border-green-200 hover:shadow-md transition-all group bg-white"
                    >
                      <div className="flex gap-4 items-center">
                        ...
                        <div>
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            <span
                              className={`px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider rounded border ${getStatusStyle(displayStatus)}`}
                            >
                              {displayStatus}
                            </span>
                          </div>
                          ...
                          <div className="flex items-center gap-3 mt-1 text-sm text-gray-500 font-medium">
                            ...
                            <span>
                              {listing.quantity || listing.weight || 0}{" "}
                              {listing.unit || "kg"}
                            </span>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {displayStatus !== "DRAFT" && (
                            <button
                              onClick={() => {
                                console.log(
                                  "Ouverture du modal pour l'annonce :",
                                  listing.id,
                                ); // Ajout d'un log pour vérifier
