...
async function callAgent(
  ...
  if (commands.length === 0) {
    ...
    if (/pending/i.test(lower))
      commands.push({ type: "highlight_all_pending" });
    if (/collected/i.test(lower))
      commands.push({ type: "highlight_all_collected" });
  ...
  const steps: AgentStep[] = [
    ...
    ...(data.results?.length
      ? [
          {
            id: "a",
            label: `${data.results.length} action(s) executed`,
            done: true,
          },
        ]
      : []),
    ...
    async (overrideText?: string) => {
      ...
      setMsgs((p) => [
        ...p,
        {
          ...
          ts: new Date(),
          text: "",
          steps: [{ id: "t", label: "Consulting data sources…", done: false }],
          thinking: true,
        },
      ]);
        ...
        setMsgs((p) =>
          p.map((m) =>
            m.id === thinkId
              ? {
                  ...m,
                  text: `⚠️ Agent error: ${e?.message ?? "Execution failed"}`,
                  thinking: false,
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                <span className="text-[10px] text-brand-green font-black tracking-widest uppercase">
                  Agent Online
                </span>
              </div>
            </div>
          ...
          <div className="flex items-end gap-3 bg-slate-50 border border-slate-100 p-2 pl-5 rounded-[1.5rem] focus-within:bg-white focus-within:border-brand-green/30 focus-within:shadow-xl focus-within:shadow-brand-green/5 transition-all">
            ...
            <div className="flex gap-1.5 p-1">
              <button
                ...
                className="w-10 h-10 rounded-lg bg-white border border-slate-100 flex items-center justify-center text-slate-400 hover:text-slate-600 shadow-sm transition-all"
              >
                <ImagePlus size={18} />
              </button>
...
const Stat = ({ badge, label, color, bg, Icon }: any) => (
  <div
    className={`flex items-center gap-2 px-3 py-1.5 rounded-xl ${bg} border border-black/[0.03] shrink-0 transition-all hover:scale-105 shadow-sm`}
  >
    <Icon size={12} className={color} />
    <div className="flex flex-col">
      <span
        className={`text-[9px] font-black uppercase tracking-widest ${color}`}
      >
        {label}
      </span>
