...
const CustMsgBubble: React.FC<CustMsgBubbleProps> = ({ msg, delay: d }) => {
  ...
  return (
    ...
    >
      {/* Avatar */}
      <div
        className={`w-9 h-9 rounded-xl shrink-0 flex items-center justify-center transition-all duration-300 shadow-sm
        ${
          isAgent
            ? "bg-linear-to-br from-brand-green to-emerald-600 text-white shadow-brand-green/20"
      ...
      <div
        className={`max-w-[85%] flex flex-col gap-2 ${isAgent ? "items-start" : "items-end"}`}
      >
          ...
          <div className="w-full bg-emerald-50/50 backdrop-blur-sm border border-emerald-100 rounded-2xl p-3 space-y-2">
            {msg.thinking ? (
              <div className="flex items-center gap-3">
                ...
                <span className="text-[10px] font-bold text-brand-green tracking-tight uppercase">
                  Searching market…
                </span>
              </div>
            ) : (
              <div className="space-y-1.5">
                {msg.steps.map((s, i) => (
                  <div
                    key={s.id}
                    className="flex items-center gap-2 opacity-0 animate-in fade-in slide-in-from-left-2 duration-300"
                    style={{
                  ...
                  >
                    ...
                    <span className="text-[10px] font-bold text-emerald-800/60 uppercase tracking-tight">
                      {s.label}
                    </span>
                  </div>
                ))}
        ...
        {msg.text && (
          <div
            className={`px-4 py-3 text-[13px] leading-relaxed font-medium wrap-break-word shadow-sm
              ${
                isAgent
            ...
            dangerouslySetInnerHTML={{
              __html: msg.text
                .replace(
                  /\*\*(.*?)\*\*/g,
                  `<strong class="${isAgent ? "text-brand-green" : "text-white"} font-black">$1</strong>`,
                )
