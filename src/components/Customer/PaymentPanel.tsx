...
const PaymentPanel: React.FC<PaymentPanelProps> = ({ point, onClose, onComplete }) => {
  ...
  const methods = [
    { id: "card",   label: "Carte Bancaire", icon: CreditCard, color: "bg-blue-600", border: "border-blue-100", highlight: "bg-blue-50" },
    { id: "momo",   label: "MTN MoMo",       icon: Smartphone, color: "bg-amber-500", border: "border-amber-100", highlight: "bg-amber-50" },
    { id: "orange", label: "Orange Money",   icon: Smartphone, color: "bg-orange-600", border: "border-orange-100", highlight: "bg-orange-50" },
  ];
      ...
      >
        ...
        <div className="p-8 pb-6 bg-gradient-to-br from-emerald-50 via-emerald-50/30 to-white border-b border-emerald-100/50">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-black text-emerald-900 tracking-tight">Finaliser l'achat</h2>
            {phase === "choose" && (
              <button 
                onClick={handleClose}
          ...
          <div className="bg-white rounded-3xl p-5 border border-emerald-100/80 shadow-sm">
            <div className="flex items-center gap-4 mb-5">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-black text-slate-900 truncate uppercase tracking-tight">{point.label}</p>
                <p className="text-[11px] font-bold text-slate-400 truncate uppercase mt-0.5 tracking-wider">{point.category}</p>
              </div>
            </div>
        ...
        <div className="p-8">
          {phase === "choose" && (
            <div className="space-y-6">
              <div>
                ...
                <p className="text-xs font-medium text-slate-400">Sélectionnez votre moyen de paiement préféré.</p>
              </div>
              ...
              <div className="space-y-3">
                {methods.map(m => (
                  ...
                  >
                    <div className={`w-12 h-12 rounded-2xl ${m.color} flex items-center justify-center text-white shadow-lg transition-transform group-hover:rotate-6`}>
                      <m.icon size={22} />
                    </div>
                    <div className="flex-1 text-left">
                      <p className={`text-sm font-black tracking-tight ${m.color.replace('bg-', 'text-')}`}>{m.label}</p>
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Sécurisé & Instantané</p>
                    </div>
                    <div className="text-right">
                       <span className={`text-base font-black ${m.color.replace('bg-', 'text-')}`}>{point.fixedPrice}</span>
          ...
          {phase === "done" && (
            <div className="py-8 text-center animate-in fade-in zoom-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="mt-6 inline-flex items-center gap-2 px-5 py-2.5 rounded-2xl bg-emerald-50 border border-emerald-100">
                <span className="text-[10px] font-black text-emerald-800/40 uppercase tracking-widest leading-none">Référence :</span>
                <span className="text-xs font-black text-emerald-700 tracking-wider leading-none">{txRef}</span>
              </div>
          ...
          {phase === "error" && (
            <div className="py-8 text-center animate-in fade-in zoom-in duration-500">
              ...
              <h3 className="text-lg font-black text-slate-900 tracking-tight uppercase">Échec du paiement</h3>
              <p className="mt-2 text-sm font-medium text-slate-400 px-6">{errMsg}</p>
