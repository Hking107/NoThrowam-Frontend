...
// On ajoute une prop pour pouvoir fermer la modale depuis l'extérieur
...
const WasteScannerModal: React.FC<WasteScannerModalProps> = ({ onClose }) => {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      // Simulation pour le test de l'UI
      setTimeout(() => {
        setAiResult({
          category: 'Trash', 
          confidence: 92.5,
  ...
  return (
    // ICI: Les classes pour l'effet Modale + Blur
        ...
        <div className="p-8 pt-4">
          {!previewUrl ? (
            <div
              ...
              className={`flex flex-col items-center justify-center p-12 border-4 border-dashed rounded-xl cursor-pointer transition-all duration-300 ease-in-out
                ${isDragging 
            ...
            >
              ...
              <h3 className="text-lg font-bold text-gray-700">
                {isDragging ? 'Relâchez pour déposer !' : 'Glissez & déposez une photo ici'}
              </h3>
            ...
            <div className="flex flex-col items-center">
              <div className="relative w-full max-w-md">
                ...
                <button 
                  onClick={handleReset}
                  className="absolute -top-3 -right-3 bg-red-500 text-white rounded-full w-8 h-8 flex items-center justify-center font-bold shadow-lg hover:bg-red-600 transition"
              ...
              {!aiResult && (
                <button
                  ...
                  className={`mt-6 w-full max-w-md py-3 rounded-xl text-white font-bold text-lg shadow-lg transition-all flex items-center justify-center gap-3
                    ${isAnalyzing ? 'bg-green-400 cursor-not-allowed' : 'bg-green-600 hover:bg-green-700 hover:shadow-green-500/30 hover:-translate-y-1'}`}
                >
                  {isAnalyzing ? (
                    <>
          ...
          <div className="bg-gray-50 border-t border-gray-100 p-8 pt-6 animate-[fadeIn_0.5s_ease-out]">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-white p-3 rounded-lg shadow-sm border border-gray-100">
                ...
                <p className={`text-lg font-bold ${aiResult.category === 'Trash' ? 'text-red-600' : 'text-blue-600'}`}>
                  {aiResult.category}
                </p>
              </div>
              ...
              <div className="bg-white p-3 rounded-lg shadow-sm border border-gray-100">
                ...
                <p className={`text-sm font-bold mt-1 ${aiResult.category === 'Trash' ? 'text-red-600' : 'text-orange-600'}`}>
                  {aiResult.action}
                </p>
              </div>
            </div>

            {aiResult.category === 'Trash' ? (
              <button 
                onClick={handleReset}
                className="mt-6 w-full bg-red-100 hover:bg-red-200 text-red-700 border border-red-300 font-bold py-3 rounded-xl shadow-sm transition flex justify-center items-center gap-2"
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>
                Déchet refusé : Analyser une autre image
              </button>
            ) : (
              <button className="mt-6 w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 rounded-xl shadow-md transition flex justify-center items-center gap-2">
