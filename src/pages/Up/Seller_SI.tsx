...
export const Seller_SI = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    localStorage.setItem("userPayload", JSON.stringify(formData));
    setLoading(true);

    try {
     // Enlève l'URL de ngrok, garde juste à partir de /api
const response = await fetch("/api/v0/auth/register/", {
  ...
  body: JSON.stringify({
      ...
      // N'oublie pas d'ajouter les champs requis par ton endpoint (ex: name, role)
